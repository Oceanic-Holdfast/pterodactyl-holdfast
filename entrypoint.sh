...
LOGS_PATH="${LOGS_DIR}/${CONFIG_PATH%.*}"
...
"${STEAMAPPDIR}/Holdfast NaW" \
  ...
  -logFile "$LOGS_PATH/outputlog.txt" \
  -logArchivesDirectory "$LOGS_PATH/logs_archive/" \
  -adminCommandsLogFilePath "$LOGS_PATH/adminactions.txt" \
  -playersLogFilePath "$LOGS_PATH/playerlogin.txt" \
  -scoreboardLogFilePath "$LOGS_PATH/scorelog.txt" \
  -chatLogFilePath "$LOGS_PATH/chatlog.txt" \
  -vacLogFilePath "$LOGS_PATH/vaclog.txt" \
